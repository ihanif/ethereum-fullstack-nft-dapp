...
it("can add the star name and star symbol properly", async () => {
  ...
  assert.equal(await instance.name.call(), "StarNotary NFT");
  assert.equal(await instance.symbol.call(), "SNF");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  await instance.createStar("Star7", tokenId1, { from: owner1 });
  await instance.createStar("Star8", tokenId2, { from: owner2 });
...
it("lets a user transfer a star", async () => {
  ...
  // 1. create a Star with different tokenId
  await instance.createStar("Star9", tokenId, { from: owner1 });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  let star = await instance.lookUptokenIdToStarInfo.call(tokenId);
  // 3. Verify if you Star name is the same
  assert.equal(star, "Star10");
});
